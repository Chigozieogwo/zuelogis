...
import { trackCreateAction } from '../actions/trackActions.js'

const TrackCreateScreen = ({ location, history }) => {
  ...
  const trackCreate = useSelector((state) => state.trackCreate)
  const { loading, error } = trackCreate
  ...
  return (
    <>
      <div
        ...
        <Link to="/admin/tracklist" className="btn btn-primary my-3">
          Go Back
        </Link>
        <FormContainer>
          <h1 style={{ textAlign: 'center' }}>Create Track</h1>
          {loading && <Loader />}
